...
function Nav() {
  ...
  return (
    <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible'>

    {showSearch && userData.role=="user" && <div className='w-[90%] h-[50px] bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%]'>
      ...
      {userData.role=="user" && <div className='md:w-[60%] lg:w-40% h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] hidden md:flex'>
       ...
       <div className='flex items-center gap-4'>
        {userData.role=="user" && showSearch?<RxCross2 className=' text-[#ff4d2d] md:hidden' onClick={()=>setShowSearch(false)}/>:<IoIosSearch size={25} className=' text-[#ff4d2d] md:hidden' onClick={()=>setShowSearch(true)}/>}

        {userData.role=="owner" && <>
            <button className='flex items-center gap-1 p-2 cursor-pointer rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d]'>
              <FaPlus size={20} />
              <span>Add Food Item</span>
            </button>
        </>}
  
        {userData.role=="user" && <div className='relative cursor-pointer flex items-center'>
